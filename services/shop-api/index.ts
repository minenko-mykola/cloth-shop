...
const app = express();
dotenv.config({ path : "envs/.env.api" , override : false });
...
const TOPIC : string = process.env.TOPIC || "reserve-api";
...
app.listen(PORT, async () => {
    await start();
    console.log(`[Shop API Service] : API Service started on port ${PORT}`);
})
